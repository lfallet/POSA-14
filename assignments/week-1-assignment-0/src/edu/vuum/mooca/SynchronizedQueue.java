 ...
 * 
 * @brief This class tests the use of Java Threads and several implementations of the Java BlockingQueue interface.
 */
public class SynchronizedQueue {
     ...
     * 
     * @brief Enumerated type for return values of testing logic, has String for easy output.
     */
    public enum SynchronizedQueueResult {
        RAN_PROPERLY("Threads Ran Properly."), JOIN_NEVER_CALLED("Join() never called."), THREADS_NEVER_RAN("Threads never ran."), THREADS_NEVER_INTERUPTED(
                "Threads never interrupted."), THREADS_THREW_EXCEPTION("Thread threw an exception."), THREADS_NEVER_CREATED(
                "Threads never created."), TESTING_LOGIC_THREW_EXCEPTION("Testing Logic threw Exception."), THREADS_TIMEDOUT(
     ...
     * @class QueueAdapter
     * 
     * @brief Applies a variant of the GoF Adapter pattern that enables us to test several implementations of the BlockingQueue interface.
     */
    ...
    /**
     * This runnable loops for mMaxIterations and calls put() on mQueue to insert the iteration number into the queue.
     */
        ...
        public void run() {
            for (int i = 0; i < mMaxIterations; i++)
                try {
                    ...
                    System.out.println("Thread properly interrupted by " + e.toString() + " in producerRunnable");
                    // This isn't an error - it just means that
                    // we've been interrupted by the main Thread.
                    return;
                } catch (TimeoutException e) {
                    System.out.println("Exception " + e.toString() + " occurred in producerRunnable");
                    // Indicate a timeout.
        ...
        public void run() {
            for (int i = 0; i < mMaxIterations; i++)
                try {
                    ...
                    System.out.println("Thread properly interrupted by " + e.toString() + " in consumerRunnable");
                    // This isn't an error - it just means that
                    // we've been interrupted by the main Thread.
                    return;
                } catch (TimeoutException e) {
    ...
    /**
     * Number of iterations to test (the actual test shouldn't run this many iterations since the Threads ought to be interrupted long
